...
from app.database import get_connection, insert_app, insert_user
...
@router.get("/apps/upload_csv")
def upload_csv_app_form(request: Request):
    return templates.TemplateResponse("upload_csv.html", {"request": request, "entity": "apps"})

@router.post("/apps/upload_csv")
async def upload_csv_app(file: UploadFile = File(...)):
    await process_uploaded_csv(file, "Apps")
    return RedirectResponse(url="/apps", status_code=303)
...
@router.get("/users/upload_csv")
def upload_csv_user_form(request: Request):
    return templates.TemplateResponse("upload_csv.html", {"request": request, "entity": "users"})

@router.post("/users/upload_csv")
async def upload_csv_user(file: UploadFile = File(...)):
    await process_uploaded_csv(file, "Users")
    return RedirectResponse(url="/users", status_code=303)
